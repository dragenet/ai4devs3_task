...
import type { DocHeaders, IDoc } from './textSplitter.types';
...
export class TextSplitterService {
  ...
  async initializeTokenizer(model?: string): Promise<void> {
  ...
  async split(
    ...
    const currentHeaders: DocHeaders = {};
  ...
  private getChunk(
    ...
    while (tokens + overhead > limit && end > start) {
      console.log(
        `Chunk exceeds limit with ${tokens + overhead} tokens. Adjusting end position...`,
      );
  ...
  private extractHeaders(text: string): DocHeaders {
  ...
  private updateCurrentHeaders(current: DocHeaders, extracted: DocHeaders): void {
  ...
  private clearLowerHeaders(headers: DocHeaders, level: number): void {
